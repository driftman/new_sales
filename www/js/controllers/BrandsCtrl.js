app.controller('BrandsCtrl', function ($scope, $timeout, $state, total, ca, Prelevements, Retours, CallSteps, CartUtilities, $log, $ionicLoading, Chargement, Commandes, IonicPopUpUtilities, $ionicPopup, position, LigneCommandes, $stateParams, Articles, $ionicModal, Missions, $filter) {
        ...
        var echange = $scope.echange = $stateParams.echange == "true" ? true : false;
        var sortant = $scope.sortant = $stateParams.output == "1" ? true : false;
        var entrant = $scope.entrant = $stateParams.input == "1" ? true : false;

        var logs = [ ["controller", "echange", "sortant", "entrant", "params"], ["BrandsCtrl", echange, sortant, entrant, $stateParams] ];
        console.table(logs);
        ...
        if (forChargement) {
            ...
            $scope.title = "Prélévement stock client";
        }
        else if (retour) {
            $scope.title = "Retours client";
        } else if (echange) {
            if(entrant) {
                $scope.title = "Echange entrant";
            } else {
                $scope.title = "Echange sortant";
            }
        } else {
            $scope.title = "Marques";
        }
        ...
        $scope.entrantAction = function() {
            $state.go("app.in");
        };

        $scope.sortantAction = function() {
            $state.go("app.out");
        };
        ...
        $scope.sendPrelevement = function () {
            var popup = $ionicPopup.alert({
                ...
                template: '<div style="font-size: 13px;" > Etes-vous sûr de vouloir confirmer ?</div>'
            })
        ...
        $scope.sendDemande = function () {
            ...
            if (typeof(cart.items) != "undefined" && cart.items.length > 0) {
                ...
                $ionicPopup.alert(IonicPopUpUtilities.alert("Erreur", "Aucune ligne n'a été ajoutée"));
            }
        ...
        Articles.getMarques(exclusion, isVendeur, forChargement, retour, echange, sortant, entrant).then(
            function (marques) {
                ...
                if (typeof(_marques.exclusion) == "undefined") {
                    if (exclusionCount === count && count != 0 && !prelevement && !retour && !forChargement) {
