app.controller('AnalyseCtrl', function ($scope, $stateParams, $ionicLoading, $timeout, ionicMaterialInk, ionicMaterialMotion, $log, CartUtilities, $state, Clients, $filter) {
        $log.log("erzer");
        ...
        $scope.totalTTC = $scope.mission.totalTTC || 0;
        $scope.objectifCA = $scope.profile.objectifCA;
        ...
        /*Clients.getClient(JSON.parse(window.localStorage["mission"]).client_id).then(function (success) {
            ...
            $scope.fillStar = $scope.isfillStar();
            
            console.log($scope.fillStar, "fillstar", $scope.client.golden_store, $scope.consumedSbds, $scope.client);

        });*/
        
        $scope.isfillStar=function(){
           var goldenPointsPercentage= ($scope.consumedSbds/$scope.sbds.length)*100;
           var promotionsPercentage=($scope.consumedPromotions/ $scope.promotions.length )*100;
           return $scope.sbds.length!=0?goldenPointsPercentage>=90:promotionsPercentage>=50;
        }
        ...
        $scope.caRatio = $scope.ca > 0 ? $filter("number")(($scope.totalTTC/ $scope.infos.objectif_ca) * 100, 2) + "%" : "0.00%";
       
        $scope.checkout = function () {
            $ionicLoading.show({ template: "Finalisation de votre visite en cours ..."});
